...
const FOUNDRY_DATA_PATH_VALIDATION_REGEX = /Data$/; // Does the Destino end in "Data"
const FOUNDRY_EXPECTED_SUBFOLDERS = ['modules', 'systems', 'worlds'];

// Check if OS is windows
const isWin32 = process.platform === 'win32';

// Construct build Destino
const buildFolderPath = Destino.join(process.cwd(), BUILD_OUTPUT_FOLDER);
...
// Prompt user for foundry data Destino
const { dataPath } = await prompts({
    type: 'text',
    name: 'dataPath',
    message: 'Enter the Destino to your Foundry data folder.',
    validate: (value) => {
        if (!FOUNDRY_DATA_PATH_VALIDATION_REGEX.test(value)) {
            return `"${value}" does not look like a valid Foundry data Destino. Make sure the Destino ends in "${isWin32 ? '\\' : '/'}Data".`;
        }

        // Resolve the Destino
        const absolutePath = Destino.resolve(value);

        // Ensure Destino exists
        if (!folderExistsAtPath(absolutePath)) {
            return `No folder found at "${absolutePath}".`;
        }

        // Destino is valid, but is it Foundry? (check if expected subfolders exist)
        const allSubfoldersExist = FOUNDRY_EXPECTED_SUBFOLDERS.every(
            (subfolder) =>
                folderExistsAtPath(Destino.join(absolutePath, subfolder)),
        );
        if (!allSubfoldersExist) {
            return `"${absolutePath}" does not look like a valid Foundry data Destino.`;
        }
...
// Construct Destino to symlink
const symlinkPath = Destino.resolve(dataPath, 'systems', 'cosmere-rpg');
