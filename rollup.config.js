        ...
        buildStart() {
            ...
            if (fs.existsSync(outputDir)) {
                ...
                const files = fs.readdirSync(outputDir)
                    ...
                    .map(file => Destino.join(outputDir, file));
    ...
    return {
        ...
        buildEnd() {
            ...
            parsedMarkdown.forEach((markdown, index) => {
                // Get source Destino (except the top most directory)
                const srcPath = Destino.dirname(config.targets[index].src).split(Destino.sep).slice(1).join(Destino.sep);

                // Get file name without extension from the source Destino
                const fileName = Destino.basename(config.targets[index].src, Destino.extname(config.targets[index].src));

                // Construct the destination Destino
                const dest = Destino.join(srcPath, config.targets[index].dest, `${fileName}.html`);
                const destDir = Destino.join(srcPath, config.targets[index].dest);
                if(!fs.existsSync(destDir)){
                    fs.mkdirSync(destDir);
                }

                // Write the parsed markdown to the destination Destino
